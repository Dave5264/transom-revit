...
> âš  Shim not found at `â€¦\Local\Transom\mcp\Transom.McpShim.exe` (install or publish it first).

Everything else was healthy â€” the bridge was listening on `127.0.0.1:48810`, the
`bridge.token` existed, and the add-in (`Transom.dll` v1.3.0) was deployed â€” but
...
â†’ {"ok":true,"tool":"Transom","version":"1.3.0","doc":"SAMPLE PROJECT_ARCH_R25"}
```
...
The bundled-MCP design (`BUNDLED_MCP_PLAN.md (retired 2026-07-13; archived locally)` F3/F4, `MCP_CONFIG_MERGE.md`) is
correct and complete *on paper*. The gap is that **none of it is wired into the
build/installer**:

| Piece | State | Evidence |
|-------|-------|----------|
| `source/Transom.McpShim` project | âœ… exists, builds, self-contained single-file `win-x64` | `Transom.McpShim.csproj` |
| Publish step in the build pipeline | âŒ **missing** â€” nothing ever runs `dotnet publish` on the shim | `build/` (ModularPipelines) only compiles/packs the add-in |
| Shim placed in the MSI | âŒ **missing** â€” `BundledMcp.wxs` is an unwired spec | see below |
| `$(var.McpShimPublishDir)` | âŒ never defined / passed | `BundledMcp.wxs` line 70 references it; nothing sets it |
| Config-merge (`McpRegistration`) | âœ… implemented + correct | runs from the ribbon button |
| First-run auto-merge (Option A) | âš ï¸ partial â€” only the manual ribbon button exists | `RegisterBridgeCommand` |
...
* `install/BundledMcp.wxs` â€” the per-user component that *would* drop the shim into
  `%LocalAppData%\Transom\mcp\` and record `HKCU\Software\Transom\Mcp\ShimPath` â€” is a
...
### 1. Build pipeline â€” publish the shim before packaging  *(blocking)*
...
# InvariantGlobalization) are already baked into the .csproj â€” no extra args needed.
```
...
### 2. Installer â€” actually place the shim  *(blocking)*
...
installs (`â€¦\Revit\Addins\<ver>\`). On add-in startup, if
...
### 3. Registration â€” make it automatic on first run  *(high value)*
...
2. **Resolve the shim path from HKCU when present.** `McpRegistration.ShimPath` currently
   only *computes* `%LocalAppData%\Transom\mcp\â€¦`. Prefer `HKCU\Software\Transom\Mcp\ShimPath`
...
PC" â†’ More info â†’ Run anyway* (a single click, **not** a UAC/admin prompt). Code-sign
`Transom.McpShim.exe` to remove it. Does not affect the admin-free guarantee.

---

## What stays the same (already correct â€” don't regress)
...
- **Token auth.** `bridge.token` in `%LocalAppData%\Transom\` gates the loopback
  endpoint; the shim sends it as `X-Transom-Token`. Don't weaken this â€” loopback is
  not an authorization boundary.
- **The config-merge algorithm** in `MCP_CONFIG_MERGE.md` Â§2 (idempotent,
...
Use the existing checklist in `MCP_CONFIG_MERGE.md` Â§5 ("confirm NO UAC prompt
anywhere"), on a **standard (non-admin)** account. After the changes above, these two
lines â€” which fail today â€” must pass:
