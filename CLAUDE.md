...
| `source/Transom/Core/BridgeServer.cs` / `BridgeEventHandler.cs` / `BridgeTools.cs` | The in-Revit loopback HTTP bridge (TcpListener on 48810) + the tool dispatch. **5 tools:** `status`, `list_schedules`, `read_schedule`, `set_parameter`, `set_parameters`. |
