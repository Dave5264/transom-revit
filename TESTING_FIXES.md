...
**Held — #3 (UJ multi-type "group header").** The safe detection touches the export's anchor-resolution mechanism (`ScheduleReader.ReadTypeAnchors`/`ReadAnchorColumn`), which is round-trip-critical, and the approach is a genuine fork (restyle-and-note vs. split-into-two-round-trippable-rows). Deferred to its own focused change + round-trip test rather than riding this batch.
